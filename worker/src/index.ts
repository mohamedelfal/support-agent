 ...
 * يعتمد على AIChatAgent مع durable_objects و migrations
 * ============================================================
 */
...
export class SupportAgent extends AIChatAgent<Env> {
  ...
  async onChatMessage() {
    ...
    const systemPrompt = `أنت مساعد دعم فني محترف، ودود، ومختصر.
قواعدك الأساسية:
- أجب بالعربية الفصحى، وبجمل مختصرة وواضحة.
- استخدم الأدوات المتاحة عند الطلب.
- لا تكرر نفس الإجابة.
- إذا لم تعرف الإجابة، قل "لا أملك هذه المعلومة".`;
