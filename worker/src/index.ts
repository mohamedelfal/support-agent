// ============================================================
// وكيل دعم عملاء - الإصدار النهائي المتقن (Llama 3.2 3B)
// ============================================================
...
// ============================================================
// 🤖 الوكيل الذكي - الإصدار النهائي المتقن
// ============================================================
app.post('/api/ask', async (c) => {
    try {
        ...
        // ============================================================
        // 2. 🔥 استرجاع السياسات (رد مباشر من قاعدة المعرفة)
        // ============================================================
        ...
        for (const word of words) {
            ...
            if (knowledgeResults.results && knowledgeResults.results.length > 0) {
                ...
                // نتحقق من أن التطابق قوي
                if (k.question.includes(word) || (k.keywords && k.keywords.includes(word))) {
        ...
        // إذا وجدنا سياسة، نرد مباشرة (بدون استدعاء النموذج)
        if (foundKnowledge) {
            await db.prepare(
        ...
        // ============================================================
        // 3. بناء System Prompt محسّن (مع دقة لغوية وإيجاز)
        // ============================================================
        const systemPrompt = `أنت وكيل دعم عملاء محترف في شركة عالمية.

**شخصيتك:**
- ودود، مهذب، ومتفهم.
- تقدم إجابات دقيقة، واضحة، ومختصرة.
- تستخدم اللغة العربية الفصحى الصحيحة إملائياً ونحوياً.

**تعليماتك الأساسية:**
- أجب على سؤال العميل بإجابة مختصرة وواضحة (لا تتجاوز 3-4 جمل).
- تأكد من صحة الإملاء والنحو في ردودك.
- إذا كان السؤال يتعلق بسياسات الشركة (الشحن، الاسترجاع، الحساب)، اعتذر بلطف واطلب التواصل مع الدعم البشري.
- للأسئلة العامة، قدم تعريفاً واضحاً ومفهوماً.
- لا تقل "أنا نموذج لغوي"، بل قل "أنا وكيل الدعم الفني".

**مراجع مفيدة:**
- الذكاء الاصطناعي (AI): مجال في علوم الحاسوب يهدف إلى إنشاء أنظمة قادرة على محاكاة الذكاء البشري.
- وكيل الذكاء الاصطناعي (AI Agent): برنامج يستخدم الذكاء الاصطناعي لأداء مهام محددة بشكل مستقل أو شبه مستقل، مثل خدمة العملاء أو تحليل البيانات.
        ...
        try {
            response = await c.env.AI.run(
                '@cf/meta/llama-3.2-3b-instruct',
                {
                    ...
                    temperature: 0.5,          // ✅ أقل للإيجاز والدقة
                    max_tokens: 512,           // ✅ إجابات مختصرة
                    top_p: 0.9,
